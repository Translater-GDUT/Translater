...
public class TranslatorForBaidu implements Translator{
    ...
    TranslatorForBaidu(String appId, String key){
        this.appId = appId;
        this.key = key;
        this.salt = Integer.toString(Math.abs(new Random(System.currentTimeMillis()).nextInt()));
    }
    ...
    public String translateWord(String word, String from, String to) {
        ...
        String result;
        String str = WebRequest.get("https://api.fanyi.baidu.com/api/trans/vip/translate?"
                + form, "UTF-8");
